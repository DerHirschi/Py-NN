...
def confirm_I_Frames(conn_id, inp):
    ...
    no_ack = ax_conn[conn_id]['noAck']
    tx_buff = ax_conn[conn_id]['tx']
    if no_ack:
        ind_val = (inp['ctl']['nr'] - 1) % 8
        ...
        while not p_end:
            ...
            if i.upper() == 'T':
                ...
                for c in list(range(7)):
                    I_TX(list(ax_conn.keys())[0], (str(c) + '\r'))
                print('Ok ..')
            elif i.upper() == 'D':
